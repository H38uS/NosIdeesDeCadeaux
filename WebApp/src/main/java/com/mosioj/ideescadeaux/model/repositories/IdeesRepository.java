...
public class IdeesRepository extends AbstractRepository {
    ...
    private static StringBuilder getIdeaBasedSelect() {
        ...
        query.append(MessageFormat.format("  left join {0} p on i.{1} = p.{2} ",
                                          ...
                                          PrioritesColumns.ID));
        query.append(MessageFormat.format("  left join {0} c on i.{1} = c.{2} ", CategoriesRepository.TABLE_NAME, IdeeColumns.TYPE, cNom));
        query.append(MessageFormat.format("  left join {0} u on u.id = i.{1} ", UsersRepository.TABLE_NAME, IdeeColumns.RESERVE));
        query.append(MessageFormat.format("  left join {0} u1 on u1.id = i.{1} ", UsersRepository.TABLE_NAME, IdeeColumns.OWNER));
        query.append(MessageFormat.format("  left join {0} u2 on u2.id = i.{1} ", UsersRepository.TABLE_NAME, IdeeColumns.SURPRISE_PAR));
    ...
    public static Idee getIdeaWithoutEnrichmentFromGroup(int groupId) throws SQLException {
        ...
        StringBuilder query = getIdeaBasedSelect();
        query.append(MessageFormat.format(" where i.{0} = ( ", IdeeColumns.ID));
        query.append(MessageFormat.format("    select {0} from {1} where {2} = ?", IdeeColumns.ID, TABLE_NAME, IdeeColumns.GROUPE_KDO_ID));
        query.append(" ) ");
    ...
    public static List<User> getPotentialGroupUser(int groupId, int userId) throws SQLException {
        ...
        query.append(MessageFormat.format("    on ur.{0} = i.{1} \n", UserRelationsColumns.FIRST_USER, IdeeColumns.OWNER));
    ...
    public static void toutDereserver(int idea) throws SQLException {
        ...
        int groupId;
        try {
            groupId = getDb().selectInt("select " + IdeeColumns.GROUPE_KDO_ID + " from " + TABLE_NAME + " where " + IdeeColumns.ID + " = ?",
                                        idea);
    ...
    public static void removeAll(int userId) {
        getDb().executeUpdate(MessageFormat.format("delete from {0} where {1} = ?", TABLE_NAME, IdeeColumns.OWNER), userId);
        getDb().executeUpdate(MessageFormat.format("delete from {0} where {1} = ?", "IDEES_HIST", IdeeColumns.OWNER), userId);
    }
     ...
     */
    public static void fillAUserIdea(User user, Idee idee, Device device) throws SQLException {
