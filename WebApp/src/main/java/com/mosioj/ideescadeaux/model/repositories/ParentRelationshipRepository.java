...
import com.mosioj.ideescadeaux.model.repositories.columns.ParentRelationshipColumns;
...
public class ParentRelationshipRepository extends AbstractRepository {
    ...
    public static List<User> getParents(int childId) throws SQLException {
        ...
        String sb = MessageFormat.format(" select u.{0}, u.{1}, u.{2}, u.{3} ",
                    ...
                    MessageFormat.format("   from {0} t ", TABLE_NAME) +
                    MessageFormat.format("   left join {0} u", UsersRepository.TABLE_NAME) +
                    MessageFormat.format("     on u.{0} = t.{1}", UsersColumns.ID, ParentRelationshipColumns.PARENT_ID) +
    ...
    public static void deleteParents(User child) {
        getDb().executeUpdate(MessageFormat.format("delete from {0} where {1} = ?", TABLE_NAME, ParentRelationshipColumns.CHILD_ID), child.id);
    }
