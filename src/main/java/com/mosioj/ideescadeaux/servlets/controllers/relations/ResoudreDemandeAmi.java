...
public class ResoudreDemandeAmi extends IdeesCadeauxPostServlet<PeutResoudreDemandesAmis> {
	...
	public void ideesKDoPOST(HttpServletRequest request, HttpServletResponse response) throws ServletException, SQLException {
		...
		policy.getChoiceParameters().forEach((fromUserId, accept) -> processRequest(accepted, toBeRemoved, fromUserId, accept));

		// Suppression des notifications
		toBeRemoved.forEach(n -> model.notif.remove(n.id));
	...
	protected void processRequest(List<User> accepted, Set<AbstractNotification> toBeRemoved, int fromUserId, boolean accept) {
		...
		if (!model.userRelationRequests.associationExists(fromUserId, thisOne.id)) {
		...
		try {
			if (accept) {
				logger.info(MessageFormat.format("Approbation de la demande par {0} de l'utilisateur {1}.", thisOne.id, fromUserId));
				model.userRelations.addAssociation(fromUserId, thisOne.id);
				model.userRelationRequests.cancelRequest(fromUserId, thisOne.id);
				...
				logger.info(MessageFormat.format("Refus de la demande par {0} de l'utilisateur {1}.", thisOne.id, fromUserId));
				model.userRelationRequests.cancelRequest(fromUserId, thisOne.id);
		...
		toBeRemoved.addAll(model.notif.getNotifications(thisOne.id,
