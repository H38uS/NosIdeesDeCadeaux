...
import static com.mosioj.ideescadeaux.model.repositories.columns.UserRelationsSuggestionColumns.SUGGESTED_BY;
import static com.mosioj.ideescadeaux.model.repositories.columns.UserRelationsSuggestionColumns.SUGGESTED_TO;
import static com.mosioj.ideescadeaux.model.repositories.columns.UserRelationsSuggestionColumns.SUGGESTION_DATE;
import static com.mosioj.ideescadeaux.model.repositories.columns.UserRelationsSuggestionColumns.USER_ID;
...
public class UserRelationsSuggestionRepository extends AbstractRepository {
    ...
    public static boolean hasReceivedSuggestion(int userId) throws SQLException {
        return getDb().doesReturnRows(MessageFormat.format("select 1 from {0} where {1} = ?", TABLE_NAME, SUGGESTED_TO),
                                      userId);
    }
        ...
        String query = MessageFormat.format(
                ...
                UsersColumns.AVATAR) +
                       ...
                       MessageFormat.format(" inner join {0} u3 on u3.{1} = urs.{2} ",
                                            ...
                                            UsersColumns.ID,
                                            USER_ID) +
                       MessageFormat.format(" where {0} = ? ", SUGGESTED_TO) +
