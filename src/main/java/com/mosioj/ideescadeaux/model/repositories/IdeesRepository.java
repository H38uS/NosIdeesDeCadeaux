...
public class IdeesRepository extends AbstractRepository {
    ...
    private static StringBuilder getIdeaBasedSelect() {
        ...
        query.append(MessageFormat.format("  left join {0} p on i.{1} = p.{2} ",
                                          ...
                                          PRIORITE,
                                          PrioritesColumns.ID));
        query.append(MessageFormat.format("  left join {0} c on i.{1} = c.{2} ", CategoriesRepository.TABLE_NAME, TYPE, cNom));
        query.append(MessageFormat.format("  left join {0} u on u.id = i.{1} ", UsersRepository.TABLE_NAME, RESERVE));
        query.append(MessageFormat.format("  left join {0} u1 on u1.id = i.{1} ", UsersRepository.TABLE_NAME, OWNER));
        query.append(MessageFormat.format("  left join {0} u2 on u2.id = i.{1} ", UsersRepository.TABLE_NAME, SURPRISE_PAR));
